<template>
	<section class="aui-flexView">
		<header class="aui-navBar aui-navBar-fixed">
			<navigator class="aui-navBar-item" >
			...
			<view class="aui-tab" data-ydui-tab>
				...
				<view class="tab-panel">
					<block v-for="(menuList,index2) in menuLists" :key="index2">
						<view :class="[currentTab==index2 ? 'tab-panel-item tab-active' : 'tab-panel-item']">
							<block v-for="(menuList2,index3) in menuList" :key="index3">
								<view class="tab-item">
									<navigator class="aui-well-item aui-well-item-clear">
									
										<view class="aui-well-item-bd">
											<span>{{menuList2.dname}}</span>
										</view>
										<span class="aui-well-item-fr">{{menuList2.zt}}</span>
									</navigator>
									<view class="aui-mail-product">
										<navigator  class="aui-mail-product-time">{{menuList2.time}}</navigator>
										<navigator class="aui-mail-product-start">{{menuList2.start_point}}</navigator>
										<navigator class="aui-mail-product-end">{{menuList2.end_point}}</navigator>
									</view>
									<navigator href="javascript:;" class="aui-mail-payment">
										<p>
											实付款: ￥{{menuList2.pri}}
										</p>
									</navigator>
								</view>
								<view :class="[index3+1==menuList.length ? 'hd':'divHeight-main']"></view>
							</block>
						</view>
					</block>
				</view>
...
<style>
	...
	* {
	...
	.tab-nav-item .tab-nav-item-navigator {
	    display: inherit;
	    color: inherit;
	    font-size: 0.8rem;
	    color: #464646;
	...
	.aui-mail-product-end,.aui-mail-product-time,.aui-mail-product-start{
		padding-bottom: 6px;
		float: left;
		display: inline;
	}
	.aui-mail-product-end{
			padding-bottom: 0px;
	}
	.aui-mail-product-start::before,.aui-mail-product-end::before {
		display: inline-block;
		content: "";
		border-radius: 50%;
